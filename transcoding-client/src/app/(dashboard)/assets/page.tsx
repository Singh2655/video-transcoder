...
const Page = () => {
  ...
  useEffect(() => {
    const fetchVideo = () => {
      axios
        .get(`${process.env.SERVER}/api/v1/video/v/${fileName}`)
        .then((response) => {
  ...
  useEffect(() => {
    if (videoData) {
      const fetchVideoStatus = () => {
        axios
          .get(`${process.env.SERVER}/api/v1/video/s/${videoData._id}`)
          .then((response) => {
  ...
  return (
    <main className="flex-grow p-6 w-full min-h-screen">
        ...
        {videoData && (
          <TableBody>
            <TableRow>
              ...
              <TableCell>
                <Button onClick={handlePreview}>Play</Button>
              </TableCell>
            </TableRow>
          </TableBody>
        )}
